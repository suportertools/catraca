...
public class DataHoje_1 {
    ...
    public static String[] ArrayDataHoje() {
        ...
        result[0] = DataHoje_1.data().substring(0, 2);
        result[1] = DataHoje_1.data().substring(3, 5);
        result[2] = DataHoje_1.data().substring(6, 10);
        return result;
    }
    ...
    public static int[] ArrayDataHojeInt() {
        ...
        result[0] = Integer.parseInt(DataHoje_1.data().substring(0, 2));
        result[1] = Integer.parseInt(DataHoje_1.data().substring(3, 5));
        result[2] = Integer.parseInt(DataHoje_1.data().substring(6, 10));
        return result;
    }
    ...
    public static long calculoDosDias(Date dInicial, Date dFinal) {
        DataHoje_1.converteDataParaInteger(DataHoje_1.converteData(dInicial));
        DataHoje_1.converteDataParaInteger(DataHoje_1.converteData(dFinal));
        if (((dInicial != null) && (dFinal != null))
                && (DataHoje_1.converteDataParaInteger(DataHoje_1.converteData(dInicial)) < DataHoje_1.converteDataParaInteger(DataHoje_1.converteData(dFinal)))) {
    ...
    public String incrementarMeses(int qtd, String data) {
        if (isDataValida(data)) {
            try {
                int c = 0;
                int[] d = DataHoje_1.DataToArrayInt(data);
                if ((d[1] + qtd) > 12) {
                    int tmp = (d[1] + qtd);
                    while (tmp > 12) {
                        tmp -= 12;
...
//    }
    ...
    public String incrementarMesesUltimoDia(int qtd, String data) {
        if (isDataValida(data)) {
            try {
                int c = 0;
                int[] d = DataHoje_1.DataToArrayInt("32/" + data.substring(3, 10));
                if ((d[1] + qtd) > 12) {
                    int tmp = (d[1] + qtd);
                    while (tmp > 12) {
                        tmp -= 12;
    ...
    public String mascararData(String data) {
        return DataHoje_1.converteData(DataHoje_1.converte(data));
    }
    ...
    public static int converteDataParaInteger(String data) {
        if (isDataValida(data)) {
            return Integer.parseInt(DataHoje_1.DataToArrayString(data)[2] + DataHoje_1.DataToArrayString(data)[1] + DataHoje_1.DataToArrayString(data)[0]);
        }
        return 0;
    }
...
//                    + dif.get(GregorianCalendar.DAY_OF_MONTH) + " dia(s)");
    ...
    public boolean faixaCincoAnosApos(String data) {
        int dataHoje = DataHoje_1.converteDataParaRefInteger(DataHoje_1.data());
        int dataAntes = DataHoje_1.converteDataParaRefInteger(decrementarMeses(60, DataHoje_1.data()));
        int dataM = DataHoje_1.converteDataParaRefInteger(data);
    ...
    public int calcularIdade(String dataNascimento) {
        ...
        if (isDataValida(dataNascimento)) {
            ...
            String dataHoje = DataHoje_1.data();
            int[] dataH = DataHoje_1.DataToArrayInt(dataHoje);
            int[] dataN = DataHoje_1.DataToArrayInt(dataNascimento);
            idade = dataH[2] - dataN[2];
            int[] novaData = DataHoje_1.DataToArrayInt(incrementarAnos(idade, dataNascimento));
    ...
    public int calcularIdade(Date data) {
        ...
        return calcularIdade(DataHoje_1.converteData(data));
    }
